...
发送方使用**加密算法**将`明文`加密为`密文`，而接收方使用相应的**解密算法**将`密文`解密为`明文`。黑客只能看到`密文`，因而并不能获取到任何有用信息。如下图所示：
...
更好的方式是采用多个凯撒密码 K 轮询进行加密，比如位置为奇数的字母采用密钥 K = 2 加密，位置为偶数的字母采用密钥 K = 3 加密。![图片](img/004.png)
...
举个例子：如果将数据的比特流每 8 个比特进行分组（不足的补零），然后将所有分组进行按位`异或运算`，那么生成的结果就可以称为摘要，此算法就是一种简单的摘要算法。
...
接收方收到数据后，利用同样的摘要算法再次得到数据的`摘要`，并将其与发送方发送的`摘要`进行比对校验，如果二者不一致，则说明数据被篡改了，反之则没有。
...
在 HTTPS 协议中，当客户端与服务器通过三次握手建立 TCP 连接之后，并不会直接传输数据，而是先会经过一个 SSL/TLS 握手的过程，用于协商`会话密钥`、`鉴别密钥`以及验证证书等，之后就可以安全传输数据了！
...
4. `用于传输会话密钥的非对称加密算法`，如 ECDHE、RSA；
5. `用于验证数字证书的非对称加密算法`，如 ECDHE、RSA；
6. `用于传输数据的对称加密算法`，如 AES_128_GCM、AES_128_CBC；
7. `用于验证报文完整性的摘要算法`，如 SHA256、SHA384；
8. 格式为：**TLS_非对称加密算法_非对称加密算法_对称加密算法_摘要算法**，如果两个非对称加密算法一致，可省略不写。
