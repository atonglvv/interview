...
- List：如果列表的元素个数小于512个，列表每个元素的值都小于64字节（默认），使用ziplist(压缩链表)编码，否则使用linkedlist(双端联表)编码
- Hash：哈希类型元素个数小于512个，所有值小于64字节的话，使用ziplist(压缩链表)编码,否则使用hashtable编码。
...
# 假如 Redis 里面有 1 亿个 key，其中有10w个key是以某个固定的已知的前缀开头的，如何将它们全部找出来？
...
可以看作是一个**头插链表**，当Get某个节点的时候，会把Get的节点移动到头部。（LinkedHashMap）
...
写入快。
AOF 机制的 rewrite 模式。定期对AOF文件进行重写（合并命令），以达到压缩的目的。
...
可以使用集合，集合可以存储多个元素，但是不能重复，集合可以进行交集、并集、差集操作，从而可以实现类似。
...
为解决这种问题，Redis Cluster 提供了一种选项cluster-node-timeout（一般配置5-10s），表示当某个节点持续 timeout的时间失联时，才可以认定该节点出现故障，需要进行主从切换。如果没有这个选项，网络抖动会导致主从频繁切换(数据的重新复制)。
...
在从数据库查询数据放到缓存这一步中。建议加锁，防止大量请求并发访问数据库。
...
2.复制积压缓冲区:主节点内部维护了一个固定长度的、先进先出FIFO)队列 作为复制积压缓冲区，当主从节点offset的差距过大超过缓冲区长度时，将无法执行部分复制，只能执行全量复制。
...
- 同一分片多个节点间的数据不保持强一致性
- 读取数据时，当客户端操作的kev没有分配在该节点上时，redis会返回转向指令，指向正确的节点。
- 扩容时需要需要把旧节点的数据迁移一部分到新节点

在redis custer 架构下，每个redis 要放开两个端口号，比如一个是 6379，另外一个就是加1w 的端口号，比如16379。
...
由于sharding处理放到客户端，规模进一步扩大时给运维带来挑战客户端sharding不支持动态增删节点。服务端Redis实例群拓扑结构有变化时，每个客户端都需要更新调整。连接不能共享，当应用规模增大时，资源浪费制约优化。
